...
import { setTheme } from "../state/actions/themeActions";
import { THEMES } from "../constants";
import { green, grey, indigo } from "@material-ui/core/colors";
import { Link } from "react-router-dom";

import { Box, Button, Drawer, Fab as MuiFab, Grid, ListItem, Typography } from "@material-ui/core";
...
function Demo({ title, theme }) {
	...
	return (
		<Grid item xs={6}>
			<DemoButton active={theme === currentTheme.currentTheme} onClick={() => dispatch(setTheme(theme))}>
		...
		<Wrapper>
			...
			<Box px={4} my={3}>
				<Alert icon={false} severity="info">
					<strong>Hello!</strong> Select your style below. Choose the ones that best fits your needs.
				</Alert>
			</Box>
